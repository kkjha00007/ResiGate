...
export default function PublicVisitorEntryPage() {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <PublicVisitorEntryPageContent />
    </Suspense>
  );
}

function PublicVisitorEntryPageContent() {
  const searchParams = useSearchParams();
  const societyId = searchParams.get('societyId');
  const voiceEntryHref = societyId
    ? `/public-visitor-entry/voice?societyId=${encodeURIComponent(societyId)}`
    : '/public-visitor-entry/voice';
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-gradient-to-br from-background to-secondary p-4 py-12">
      <div className="flex flex-col items-center mb-8">
        ...
        <Link href={voiceEntryHref} className="mt-4">
          <Button size="lg" className="bg-primary text-white shadow-lg">
            Voice-Assisted Entry (for Guards/Visitors)
          </Button>
        </Link>
      </div>
